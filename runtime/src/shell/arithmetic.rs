...
use brush_parser::ast::{ArithmeticExpr, ArithmeticTarget, BinaryOperator, UnaryOperator, UnaryAssignmentOperator};
    ...
    match expr {
        ...
        ArithmeticExpr::UnaryOp(op, operand) => {
            ...
            Ok(match op {
                ...
                UnaryOperator::LogicalNot => if val == 0 { 1 } else { 0 },
        ...
        ArithmeticExpr::BinaryOp(op, left, right) => {
            ...
            Ok(match op {
                ...
                BinaryOperator::LessThan => if left_val < right_val { 1 } else { 0 },
                BinaryOperator::LessThanOrEqualTo => if left_val <= right_val { 1 } else { 0 },
                BinaryOperator::GreaterThan => if left_val > right_val { 1 } else { 0 },
                BinaryOperator::GreaterThanOrEqualTo => if left_val >= right_val { 1 } else { 0 },
                BinaryOperator::Equals => if left_val == right_val { 1 } else { 0 },
                BinaryOperator::NotEquals => if left_val != right_val { 1 } else { 0 },
...
fn get_target_value(target: &ArithmeticTarget, env: &ShellEnv) -> Result<i64, String> {
    match target {
        ...
        ArithmeticTarget::ArrayElement(name, index_expr) => {
            ...
            let value_str = env.get_array_element(name, &index.to_string())
                .unwrap_or_default();
            if value_str.is_empty() {
                Ok(0)
            } else {
...
fn set_target_value(target: &ArithmeticTarget, value: i64, env: &mut ShellEnv) -> Result<(), String> {
    match target {
        ArithmeticTarget::Variable(name) => {
