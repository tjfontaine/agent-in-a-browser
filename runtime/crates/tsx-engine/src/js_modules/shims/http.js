// http.js - Node.js http module compatible subset for WASM sandbox
// Provides HTTP client via __syncFetch__ with default port 80.
// Server APIs are stubbed (WASM sandbox cannot listen on ports).

(function () {
    var EventEmitter = globalThis.__tsxBuiltinModules.get('events');
    if (!EventEmitter) throw new Error('http module requires events module to be loaded first');

    var streamMod = globalThis.__tsxBuiltinModules.get('stream');
    if (!streamMod) throw new Error('http module requires stream module to be loaded first');

    var Readable = streamMod.Readable;
    var Writable = streamMod.Writable;
    ...
    // --- ClientRequest ---
    // Writable request object returned by http.request().
    // Extends EventEmitter (with Writable-like write/end interface).
    ...
    ClientRequest.prototype.end = function (data) {
        ...
        // Build URL — always http
        var portPart = (port === 80) ? '' : ':' + port;
        var url = 'http://' + hostOnly + portPart + path;
    ...
    // --- ServerResponse ---
    // Stub writable response for createServer handler compatibility.
    ...
    Server.prototype.listen = function () {
        throw new Error('http.Server.listen() is not supported in WASM sandbox');
    };
    ...
    function parseUrlAndOptions(urlOrOptions, optionsOrCallback, callback) {
        ...
        if (typeof urlOrOptions === 'string') {
            ...
            options = { hostname: 'localhost', port: 80, path: '/', method: 'GET' };
            cb = typeof optionsOrCallback === 'function' ? optionsOrCallback : callback;
        }
    ...
    globalThis.__tsxBuiltinModules.set('http', module);
    globalThis.__tsxBuiltinModules.set('node:http', module);
})();
