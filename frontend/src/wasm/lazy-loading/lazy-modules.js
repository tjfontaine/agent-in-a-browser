...
import { registerModule, isRegisteredCommand, isInteractiveCommand as isInteractiveCommandRegistry, getModuleForCommand as getModuleForCommandRegistry, getAllCommands, setTerminalContext, isTerminalContext, } from '@tjfontaine/wasm-loader';
 ...
 */
export function isInteractiveCommand(commandName) {
 ...
 */
export function isLazyCommand(commandName) {
 ...
 * Uses the registry if registered, otherwise checks LAZY_COMMANDS.
 */
export function getModuleForCommand(commandName) {
 ...
 */
function wrapSyncModule(syncModule) {
    return {
...
async function loadTsxEngine() {
    ...
    if (hasJSPI) {
        return wrapJspiModule(module.command);
    }
 ...
 * keyboard input.
 */
async function loadRatatuiDemo() {
    // Interactive TUI requires JSPI for stdin to work
    if (!hasJSPI) {
        throw new Error('Interactive TUI apps require JSPI (JavaScript Promise Integration). ' +
    ...
    const module = await import('@tjfontaine/wasm-ratatui/wasm/ratatui-demo.js');
 ...
 */
async function loadEdtuiModule() {
 ...
 */
export async function loadLazyModule(moduleName) {
 ...
 */
export async function loadModuleForCommand(commandName) {
 ...
 */
export function getLazyCommandList() {
 ...
 */
export function getLoadedModuleSync(moduleName) {
 ...
 */
export async function initializeForSyncMode() {
    if (hasJSPI) {
    ...
    await Promise.all(moduleNames.map(name => loadLazyModule(name).catch(err => {
